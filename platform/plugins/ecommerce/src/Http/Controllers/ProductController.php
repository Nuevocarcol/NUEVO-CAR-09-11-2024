...
class ProductController extends BaseController
{
    ...
    public function showModal(ProductRequest $request)
    {
      
      // Renderiza la vista y almacénala en una variable
    $modalHtml = view('plugins/ecommerce::products.partials.payment-modal')->render();
        dd($modalHtml);
    // Retorna la respuesta en formato JSON con el HTML renderizado del modal
    ...
    public function store(
        ...
        //$authorizateTransaction = $this->consumeCredit();
        $product = new Product();
        ...
        //dd("creando Producto",Auth::user()->super_user);
    ...
    private function consumeCredit()
    {
        if (Auth::user()->super_user != 1) {
            $result = DB::select('CALL consumirCreditos(?)', [Auth::user()->id]);
            if ($result = 'Consumido') {
                return true;
            } else {
