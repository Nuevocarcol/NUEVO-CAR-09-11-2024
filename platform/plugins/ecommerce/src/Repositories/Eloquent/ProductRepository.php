...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        $this->model = $this->model
            ...
            ->join('ec_product_label_products', 'ec_products.id', '=', 'ec_product_label_products.product_id') // proceso de ordenar por idLabel
            ->orderBy('ec_product_label_products.product_label_id', 'ASC');
                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial) {
                        ...
                        if (in_array('sku', $searchProductsBy)) {
                            $query
                                ->where(function (BaseQueryBuilder $subQuery) use ($keyword) { // @phpstan-ignore-line
                        ...
                        if (in_array('name', $searchProductsBy) || in_array('description', $searchProductsBy)) {
                            ...
                            $query
                                ->{$function}('translations', function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial) {
                                    $query->where(function (BaseQueryBuilder $subQuery) use ($keyword, $searchProductsBy, $isPartial) { // @phpstan-ignore-line
                        ...
                        if (in_array('tag', $searchProductsBy)) {
                            ...
                            $query->{$function}('tags', function (EloquentBuilder $query) use ($keyword) {
                                $query->where(function (BaseQueryBuilder $subQuery) use ($keyword) { // @phpstan-ignore-line
                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial) {
                        ...
                        if (in_array('name', $searchProductsBy) || in_array('sku', $searchProductsBy) || in_array('description', $searchProductsBy)) {
                            $query
                                ->where(function (BaseQueryBuilder $subQuery) use ($keyword, $searchProductsBy, $isPartial) { // @phpstan-ignore-line
                                    if (in_array('name', $searchProductsBy)) {
                    ...
                    $this
                        ...
                        ->whereExists(function (Builder $query) use ($attributeSet, $attributeIds) {
                            $query
                                ...
                                ->when(! EcommerceHelper::showOutOfStockProducts(), function (Builder $query) {
                                    $query
