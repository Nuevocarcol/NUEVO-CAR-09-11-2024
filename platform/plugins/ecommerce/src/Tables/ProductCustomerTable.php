...
class ProductCustomerTable extends TableAbstract
{
    ...
    {
        $data = $this->table
            ->eloquent($this->query()->where('created_by_id', Auth('customer')->user()->id))
                ...
                if ($keyword) {
                    ...
                    $query
                        ...
                        ->orWhere(function ($query) use ($keyword) {
                            $query
                                ->where('created_by_id', Auth('customer')->user()->id)
                                ->where('is_variation', 0)
