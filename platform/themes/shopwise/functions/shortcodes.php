...
app()->booted(function () {
    ...
    if (is_plugin_active('ecommerce')) {
        ...
        /**Shortcode que muestran las imagenes de las publicaciones  */
        add_shortcode('featured-publications', __('Featured Publications'), __('Featured Publications'), function (Shortcode $shortcode) {
            //$publications = get_publications();
            $id = $shortcode->idslider ?? null;
            // Filtra las publicaciones por el ID si está definido
            $publications = get_publications($id);
            return Theme::partial('shortcodes.publications.index', compact('publications', 'shortcode'));
        });
        shortcode()->setAdminConfig('featured-publications', function (array $attributes) {
            return Theme::partial('shortcodes.publications.admin-config', compact('attributes'));
        });

        /**shortcode que muestra los planes disponibles en nuevocar */
        add_shortcode('plans',__('plans'),__('plans'),function(Shortcode $shortcode){
            $planes= get_plans();
            return Theme::partial('shortcodes.plans.index',compact('planes','shortcode'));
        });
        shortcode()->setAdminConfig('plans', function (array $attributes) {
            return Theme::partial('shortcodes.plans.admin-config', compact('attributes'));
        });
        add_shortcode(
            'product-collections',
    ...
    add_shortcode('search', __('Buscador'), __('Buscador Filtro'), function (Shortcode $shortcode) {
        $brands = Brand::all();
        $categories = ProductCategory::where(['status' => 'published', 'parent_id' => 0])->get();
        return Theme::partial('shortcodes.search.index', compact('brands', 'shortcode', 'categories'));
    });

    shortcode()->setAdminConfig('search', function (array $attributes) {
        $categories = FaqCategory::query()
            ->wherePublished()
            ->pluck('name', 'id')
            ->all();

        return Theme::partial('shortcodes.search.admin-config', compact('attributes', 'categories'));
    });
});
